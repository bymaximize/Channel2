...
export default function Signup() {
  ...
  const submit = async e => {
    ...
    setBusy(true);
    try {
      const r = await API.post('/auth/signup', { username:form.username, password:form.password });
  ...
  return (
    <div style={S.page}>
      <div style={S.card}>
        ...
        <p style={S.sub}>Join Channel Pro today — it's free</p>

        {err && <div style={S.err}>⚠️ {err}</div>}

        <form onSubmit={submit} style={{display:'flex',flexDirection:'column',gap:16}}>
          {[
            ['username','Username','text','Choose a username (3+ chars)','username'],
            ['password','Password','password','Min. 6 characters','new-password'],
            ['confirm', 'Confirm Password','password','Repeat your password','new-password'],
        ...
        <p style={S.foot}>Already have an account? <Link to="/login" style={{color:'var(--accent)',fontWeight:600}}>Login</Link></p>
      </div>
    </div>
  );
}
